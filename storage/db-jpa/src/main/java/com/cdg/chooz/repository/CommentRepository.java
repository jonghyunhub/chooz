...
import com.cdg.chooz.entity.comment.Comment;
...
public interface CommentRepository extends JpaRepository<Comment,Long> {
    ...
    List<Comment> findCommentsByfilterANDGetChildren(@Param("voteId") Long voteId, @Param("gender") GenderType gender, @Param("age") AgeType age, @Param("mbti") MbtiType mbti);
    ...
    @Query("SELECT c FROM Comment c " +
            ...
            "ORDER BY p.id ASC NULLS FIRST, c.id DESC")
    List<Comment> findCommentsAllByfilter(@Param("voteId") Long voteId, @Param("gender") GenderType gender, @Param("age") AgeType age, @Param("mbti") MbtiType mbti);

    //인기순 댓글 조회
    @Query("SELECT c FROM Comment c" +
            " WHERE c.voteId = :voteId AND c.parent IS NULL AND (:gender IS NULL OR c.gender = :gender) AND (:age IS NULL OR c.age = :age) AND (:mbti IS NULL OR c.mbti = :mbti)" +
            " ORDER BY (c.likeCount + c.hateCount) DESC , c.createdDate DESC")
    List<Comment> findHotComments(@Param("voteId") Long voteId, @Param("gender") GenderType gender, @Param("age") AgeType age, @Param("mbti") MbtiType mbti, Pageable pageable);

    @Query("SELECT COUNT(c) FROM Comment c" +
            " WHERE c.voteId = :voteId AND c.parent IS NULL AND (:gender IS NULL OR c.gender = :gender) AND (:age IS NULL OR c.age = :age) AND (:mbti IS NULL OR c.mbti = :mbti)")
    int countHotComments(@Param("voteId") Long voteId,@Param("gender") GenderType gender, @Param("age") AgeType age, @Param("mbti") MbtiType mbti);

    //최신순 댓글 조회
    @Query("SELECT c FROM Comment c" +
            " WHERE c.voteId = :voteId AND c.parent IS NULL AND (:gender IS NULL OR c.gender = :gender) AND (:age IS NULL OR c.age = :age) AND (:mbti IS NULL OR c.mbti = :mbti) " +
            "ORDER BY c.createdDate DESC")
    List<Comment> findNewestComments(@Param("voteId") Long voteId, @Param("gender") GenderType gender, @Param("age") AgeType age, @Param("mbti") MbtiType mbti, Pageable pageable);

    @Query("SELECT COUNT(c) FROM Comment c" +
            " WHERE c.voteId = :voteId AND c.parent IS NULL AND (:gender IS NULL OR c.gender = :gender) AND (:age IS NULL OR c.age = :age) AND (:mbti IS NULL OR c.mbti = :mbti) ")
    int countNewestComments(@Param("voteId") Long voteId, @Param("gender") GenderType gender, @Param("age") AgeType age, @Param("mbti") MbtiType mbti);
    ...
    @Query("SELECT c FROM Comment c " +
            ...
            "ORDER BY c.id ASC")
    List<Comment> findChildComments(@Param("voteId") Long voteId, @Param("gender") GenderType gender, @Param("age") AgeType age, @Param("mbti") MbtiType mbti, @Param("parentComment") Comment parentComment);
